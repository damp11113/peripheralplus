package dev.dpsoftware.peripheralplus.common.computercraft;

import dan200.computercraft.api.peripheral.IComputerAccess;
import dan200.computercraft.api.peripheral.IPeripheral;
import net.minecraft.world.level.block.entity.BlockEntity;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

import javax.annotation.Nonnull;
import java.util.LinkedList;
import java.util.List;

/**
 * Abstract base class for ComputerCraft peripherals.
 */
public abstract class ModPeripheral<BE extends BlockEntity> implements IPeripheral {
    private final String type;
    private final BE blockEntity;
    private final List<IComputerAccess> computers = new LinkedList<>();

    public ModPeripheral(String type, @Nullable BE blockEntity) {
        this.type = type;
        this.blockEntity = blockEntity;
    }

    public void sendEvent(@Nonnull String event, @Nullable Object... arguments) {
        for (IComputerAccess pc : computers)
            pc.queueEvent(event, pc.getAttachmentName(), arguments);
    }

    @Override
    public void attach(@Nonnull IComputerAccess computer) {
        computers.add(computer);
    }

    @Override
    public void detach(@Nonnull IComputerAccess computer) {
        computers.removeIf(p -> (p.getID() == computer.getID()));
    }

    @NotNull
    @Override
    public String getType() {
        return type;
    }

    @Nullable
    public BE getTarget() {
        return blockEntity;
    }

    public static double getVersion() {
        return 0.0D;
    }

    @Override
    public boolean equals(@Nullable IPeripheral other) {
        return other == this && other.getType().equals(type) && other.getTarget() == this.getTarget();
    }
}
